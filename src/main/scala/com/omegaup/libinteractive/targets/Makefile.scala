...
class Makefile(idl: IDL, rules: Iterable[MakefileRule],
	...
	override def createWorkDirs() = {
		new File(options.outputDirectory, "libinteractive_helper").mkdir
	}

	private def generateMakefileContents() = {
		val builder = new StringBuilder
		val allRules = "libinteractive_helper/run " + rules.map(_.target).mkString(" ")
...
"""
		...
		builder ++= "libinteractive_helper/run: libinteractive_helper/run.c\n\t/usr/bin/gcc -std=c99 -o $@ -lrt $^ -O2 " +
				"-D_XOPEN_SOURCE=600 -D_BSD_SOURCE -Wno-unused-result -Wall\n\n"

		builder ++= s"run: $allRules\n\t@./libinteractive_helper/run\n"
...
"""
		...
		OutputFile("libinteractive_helper/run.c", builder.mkString)
	}
