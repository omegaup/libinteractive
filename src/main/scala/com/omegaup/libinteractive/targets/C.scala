...
class C(idl: IDL, options: Options, parent: Boolean) extends Target(idl, options) {
	...
	override def generateMakefileRules() = {
		if (parent) {
			List(MakefileRule(s"${idl.main.name}/${idl.main.name}",
				List(
					s"${idl.main.name}/${idl.main.name}.$extension",
					s"${idl.main.name}/${idl.main.name}_entry.$extension"),
				s"$compiler $cflags -o $$@ -lrt $$^ -O2 -Wl,-e__entry " +
				"-D_XOPEN_SOURCE=600 -Wno-unused-result -Wall"))
		} else {
			idl.interfaces.map(interface =>
				MakefileRule(s"${interface.name}/${interface.name}",
					List(
						s"${interface.name}/${interface.name}.$extension",
						s"${interface.name}/${interface.name}_entry.$extension"),
	...
	override def generateRunCommands() = {
		(if (parent) {
			...
			Array(s"./${interface.name}/${interface.name}")
		)
	}

	override def createWorkDirs() = {
		if (parent) {
			createWorkDir(idl.main, s"Main.$extension")
		} else {
			for (interface <- idl.interfaces) {
				createWorkDir(interface, s"${options.moduleName}.$extension")
			}
		}
	}
	...
	private def generateHeader(interface: Interface) = {
		...
		OutputFile(s"${interface.name}/${options.moduleName}.h", builder.mkString)
	}
...
"""
	...
	private def generateMainHeader() = {
		...
		OutputFile(s"${idl.main.name}/${options.moduleName}.h", builder.mkString)
	}
