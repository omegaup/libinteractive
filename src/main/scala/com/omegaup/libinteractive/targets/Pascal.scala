...
class Pascal(idl: IDL, options: Options, parent: Boolean) extends Target(idl, options) {
	override def generate() = {
		if (parent) {
			val mainFile = s"${idl.main.name}.pas"
			List(
	...
	override def generateMakefileRules() = {
		if (parent) {
			List(MakefileRule(Paths.get(idl.main.name, idl.main.name),
				List(
					Paths.get(idl.main.name, s"${idl.main.name}.pas"),
					Paths.get(idl.main.name, s"${idl.main.name}_entry.pas")),
...
"""
	...
	private def generateMainFile() = {
		val openPipes = idl.interfaces.map(interface => {
			(if (options.verbose) {
				s"""\tWriteln(ErrOutput, #9'[${idl.main.name}] opening `${pipeFilename(interface)}''');\n"""
			} else {
				""
			}) +
s"""\tif ((${pipeName(interface)} = open("${pipeFilename(interface)}", O_WRONLY)) == -1) {
		...
		exit(1);
			...
			(if (options.verbose) {
				s"""\tWriteln(ErrOutput, #9'[${idl.main.name}] opening `${pipeFilename(idl.main)}''');\n"""
			} else {
				""
			}) +
s"""\tif ((${pipeName(idl.main)} = open("${pipeFilename(idl.main)}", O_RDONLY)) == -1) {
		...
		val closePipes = (idl.interfaces ++ List(idl.main)).map(interface => {
			(if (options.verbose) {
				s"""\tWriteln(ErrOutput, #9'[${idl.main.name}] closing `${pipeFilename(interface)}''');\n"""
			} else {
				""
			}) +
